...
import { Flame, ArrowUp, Zap, Crown, Gem, Shield } from 'lucide-react';
...
export default function FeatBadge({ feat }) {
    ...
    return (
        <div className="relative group flex flex-col items-center text-center gap-2">
            ...
            <p className="font-bold text-sm text-gray-800 dark:text-gray-200">{feat.instances[0].challengeTitle}</p>
            {count > 1 && (
            ...
            <div className="absolute bottom-full mb-2 w-72 p-3 bg-gray-900 dark:bg-gray-700 text-white text-sm rounded-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
                <h4 className="font-bold border-b border-gray-600 pb-1 mb-2">{feat.instances[0].challengeTitle} ({count})</h4>
                <ul className="space-y-1 max-h-40 overflow-y-auto text-xs pr-2">
                    {feat.instances.sort((a, b) => b.date.toDate() - a.date.toDate()).map((inst, index) => (
