...
export default function RoundsTab({ league }) {
    ...
    const roundDetails = useMemo(() => {
        ...
        const playerScoresSorted = Object.keys(scores).map(uid => ({
            uid,
            name: league.members[uid]?.teamName || 'Desconocido',
            username: league.members[uid]?.username, // <-- Obtenemos el username
            points: scores[uid]
        })).sort((a, b) => b.points - a.points);
        
        const playerScoresRanked = [];
        for (let i = 0; i < playerScoresSorted.length; i++) { let rank; if (i > 0 && playerScoresSorted[i].points === playerScoresSorted[i - 1].points) { rank = playerScoresRanked[i - 1].rank; } else { rank = i + 1; } playerScoresRanked.push({ ...playerScoresSorted[i], rank }); }
        
        const validScores = playerScoresRanked.map(s => s.points);
        const bestScore = validScores.length > 0 ? Math.max(...validScores) : 0;
        const worstScore = validScores.length > 0 ? Math.min(...validScores) : 0;
        const average = validScores.length > 0 ? (validScores.reduce((a, b) => a + b, 0) / validScores.length).toFixed(1) : 0;
